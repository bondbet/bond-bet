...
import {
  STAKE_POOL_RINKEBY_PRIZE_POOL_ADDRESS,
  STAKE_POOL_RINKEBY_TICKET_ADDRESS,
  STAKE_POOL_RINKEBY_PRIZE_STRATEGY_ADDRESS,
  STAKE_POOL_RINKEBY_UNDELYING_TOKEN_ADDRESS,
  BARN_PRIZE_POOL_RINKEBY_ADDRESS,
  BOND_UNDELYING_TOKEN_ADDRESS,
  PRIZE_STRATEGY_RINKEBY_CONTRACT_ADDRESS,
  BOND_TICKETS_RINKEBY_CONTRACT_ADDRESS
...
const App = ({
  ...
  const connectWalletHandler = useCallback(async () => {
    try {
      ...
      const address = newProvider.selectedAddress
        ? newProvider.selectedAddress
        : newProvider?.accounts[0];

      setConnectedWalletAddress(address);
      setConnectedNetwork(network.name);
      setConnectedWalletName(
        library.connection.url === "metamask" ? "MetaMask" : "WalletConnect"
      );
      setConnected(true);
      ...
      alert(
        "Something went wrong when connecting the contracts. Please check your connected network."
      );
    }
  });
    ...
    <div>
      ...
      >
        ...
        <PoolContractsResolver
          ...
          ticketsAddress={BOND_TICKETS_RINKEBY_CONTRACT_ADDRESS}
        ></PoolContractsResolver>
        ...
        <PoolContractsResolver
          ...
          ticketsAddress={STAKE_POOL_RINKEBY_TICKET_ADDRESS}
        ></PoolContractsResolver>
...
const mapDispatchToProps = (dispatch) => {
  ...
  return {
    setPrizePoolContract: (value) =>
      dispatch({ type: ACTION_TYPE.PRIZE_POOL_CONTRACT, value, poolType }),
    setMainAssetTokenContract: (value) =>
      dispatch({
        type: ACTION_TYPE.MAIN_ASSET_TOKEN_CONTRACT,
        value,
        poolType,
      }),
    setTicketsContract: (value) =>
      dispatch({ type: ACTION_TYPE.TICKETS_CONTRACT, value, poolType }),
    setPrizeStrategyContract: (value) =>
      dispatch({ type: ACTION_TYPE.PRIZE_STRATEGY_CONTRACT, value, poolType }),
    setMainAssetContract: (value) =>
      dispatch({ type: ACTION_TYPE.MAIN_ASSET_CONTRACT, value, poolType }),
    setProvider: (value) => dispatch({ type: ACTION_TYPE.PROVIDER, value }),
    setConnected: (value) => dispatch({ type: ACTION_TYPE.CONNECTED, value }),
    setConnectedNetwork: (value) =>
      dispatch({ type: ACTION_TYPE.CONNECTED_NETWORK, value }),
    setConnectedWalletAddress: (value) =>
      dispatch({ type: ACTION_TYPE.CONNECTED_WALLET_ADDRESS, value }),
    setConnectedWalletName: (value) =>
      dispatch({ type: ACTION_TYPE.CONNECTED_WALLET_NAME, value }),
