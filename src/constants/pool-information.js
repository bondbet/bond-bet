export const POOL_INFORMATION = {
  COMMUNITY_REWARD_POOL: {
    TITLE: "Community Reward Pool - Bond.Bet",
    URL: "community-reward-pool",
    YIELD_SOURCE: "BarnBridge DAO Staking",
    DESCRIPTION1:
      "The Community Reward Pool is set up by BOND founders and the weekly prize in this pool is provided from BOND Community Rewards.",
    DESCRIPTION2:
      "Each week the protocol randomly chooses one winner who gets all the sum of the prize. The staked amount of BOND tokens can be withdrawn at any time without any time lockups.",
  },
  STAKE_POOL: {
    TITLE: "BB DAO Staking Pool - Bond.Bet",
    URL: "staked-pool",
    YIELD_SOURCE: "BarnBridge DAO Staking",
    DESCRIPTION1:
      "The BarnBridge DAO Staking Pool is pulling the funds of depositors together and staking them in the BarnBridge DAO Staking contract.",
    DESCRIPTION2:
      "The accrued interest is then rewarded to 3 random winner while everyone keeps their principal.",
  },
};
